...
interface Point { // Para el generador de líneas D3
  x: number;
  y: number;
}

const getTeamDetails = (teamid: number | null | undefined, allTeams: Team[]): TeamDetails => {
  if (teamid === null || teamid === undefined) return { name: 'TBD', logo: '' };
  const team = allTeams.find((t) => t.teamid === teamid);
  return team ? { name: team.teamname, logo: team.teamlogo } : { name: 'TBD', logo: '' };
};
...
const PlayoffsBracketD3 = () => {
  ...
  const { nodes, links, titles, svgDimensions, nodeStyle } = useMemo(() => {
    const allMatchesInput: Record<string, MatchData> = {
      'ub-sf-1': { ...playoffsData.upperBracket.semifinals[0], originalId: 'ub-sf-1' },
      'ub-sf-2': { ...playoffsData.upperBracket.semifinals[1], originalId: 'ub-sf-2' },
      'lb-qf-1': { ...playoffsData.lowerBracket.quarterfinals[0], originalId: 'lb-qf-1' },
      'lb-qf-2': { ...playoffsData.lowerBracket.quarterfinals[1], originalId: 'lb-qf-2' },
      'ub-final': { ...playoffsData.upperBracket.final, originalId: 'ub-final' },
      'lb-sf': { ...playoffsData.lowerBracket.semifinal, originalId: 'lb-sf' },
      'lb-final': { ...playoffsData.lowerBracket.final, originalId: 'lb-final' },
    ...
    calculatedTitles.push({ text: "Upper Bracket Semifinals", x: col0X + conf.nodeWidth / 2, y: currentY });
    currentY += conf.titleAreaHeight;
    calculatedNodes['ub-sf-1'] = { data: allMatchesInput['ub-sf-1'], x: col0X, y: currentY };
    currentY += conf.nodeHeight + conf.verticalGapNodes;
    calculatedNodes['ub-sf-2'] = { data: allMatchesInput['ub-sf-2'], x: col0X, y: currentY };
    currentY += conf.nodeHeight + conf.verticalGapSections;

    calculatedTitles.push({ text: "Lower Bracket Quarterfinals", x: col0X + conf.nodeWidth / 2, y: currentY });
    currentY += conf.titleAreaHeight;
    calculatedNodes['lb-qf-1'] = { data: allMatchesInput['lb-qf-1'], x: col0X, y: currentY };
    currentY += conf.nodeHeight + conf.verticalGapNodes;
    calculatedNodes['lb-qf-2'] = { data: allMatchesInput['lb-qf-2'], x: col0X, y: currentY };
    ...
    const lbsfTargetY = (calculatedNodes['lb-qf-1'].y + conf.nodeHeight / 2 + calculatedNodes['lb-qf-2'].y + conf.nodeHeight / 2) / 2 - conf.nodeHeight / 2;
    calculatedTitles.push({ text: "Lower Bracket Semifinal", x: col1X + conf.nodeWidth / 2, y: lbsfTargetY - conf.titleAreaHeight - 5}); // -5 para pequeño ajuste visual
    calculatedNodes['lb-sf'] = { data: allMatchesInput['lb-sf'], x: col1X, y: lbsfTargetY };
    ...
    const ubfTargetY = (calculatedNodes['ub-sf-1'].y + conf.nodeHeight / 2 + calculatedNodes['ub-sf-2'].y + conf.nodeHeight / 2) / 2 - conf.nodeHeight / 2;
    calculatedTitles.push({ text: "Upper Bracket Final", x: col2X + conf.nodeWidth / 2, y: ubfTargetY - conf.titleAreaHeight -5 });
    calculatedNodes['ub-final'] = { data: allMatchesInput['ub-final'], x: col2X, y: ubfTargetY };
    ...
    const minLbfY = calculatedNodes['ub-final'].y + conf.nodeHeight + conf.verticalGapSections;
    calculatedNodes['lb-final'] = { data: allMatchesInput['lb-final'], x: col2X, y: Math.max(lbfTargetY, minLbfY) };
    calculatedTitles.push({ text: "Lower Bracket Final", x: col2X + conf.nodeWidth / 2, y: calculatedNodes['lb-final'].y - conf.titleAreaHeight -5 });
    ...
    calculatedTitles.push({ text: "Grand Final", x: col3X + conf.nodeWidth / 2, y: gfTargetY - conf.titleAreaHeight -5 });
    calculatedNodes['grand-final'] = { data: allMatchesInput['grand-final'], x: col3X, y: gfTargetY };

    // --- Definición de Links ---
    // UBSF -> UBF
    calculatedLinks.push({ source: 'ub-sf-1', target: 'ub-final', type: 'winner', sourcePort: 'center', targetPort: 'top' });
    calculatedLinks.push({ source: 'ub-sf-2', target: 'ub-final', type: 'winner', sourcePort: 'center', targetPort: 'bottom' });
    // LBQF -> LBSF
    calculatedLinks.push({ source: 'lb-qf-1', target: 'lb-sf', type: 'winner', sourcePort: 'center', targetPort: 'top' });
    calculatedLinks.push({ source: 'lb-qf-2', target: 'lb-sf', type: 'winner', sourcePort: 'center', targetPort: 'bottom' });
    
    // UBF (Winner) -> GF
    calculatedLinks.push({ source: 'ub-final', target: 'grand-final', type: 'winner', sourcePort: 'center', targetPort: 'top' });
    // UBF (Loser) -> LBF
    calculatedLinks.push({ source: 'ub-final', target: 'lb-final', type: 'loser', sourcePort: 'center', targetPort: 'top' });
    
    // LBSF (Winner) -> LBF
    calculatedLinks.push({ source: 'lb-sf', target: 'lb-final', type: 'winner', sourcePort: 'center', targetPort: 'bottom' });
    // LBF (Winner) -> GF
    calculatedLinks.push({ source: 'lb-final', target: 'grand-final', type: 'winner', sourcePort: 'center', targetPort: 'bottom' });

    const allNodeX = Object.values(calculatedNodes).map((n: Node) => n.x + conf.nodeWidth);
    const allNodeY = Object.values(calculatedNodes).map((n: Node) => n.y + conf.nodeHeight);
    ...
    const contentHeight = Math.max(...allNodeY, ...allTitleY.map(y => y + conf.titleAreaHeight), 0);
  ...
  useEffect(() => {
    if (!d3Container.current || !nodes || !links || !titles || Object.keys(nodes).length === 0) return;
    ...
    const g = svg.append('g').attr('transform', `translate(${svgDimensions.padding.left}, ${svgDimensions.padding.top})`);
    ...
    const linkPathGenerator = d3.line<Point>()
      .x(d => d.x)
      .y(d => d.y)
      .curve(d3.curveBumpX);
    ...
    const nodeGroups = g.selectAll('.node')
      ...
      .append('g')
      .attr('class', 'node')
      .attr('transform', ([, node]: [string, Node]) => `translate(${node.x}, ${node.y})`);
    ...
    nodeGroups.each(function([id, node]: [string, Node]) {
      ...
      const itemPadding = { x: 8, y_top_center: nodeStyle.height * 0.25, y_bottom_center: nodeStyle.height * 0.75 };
      const logoSize = 16;
      const textOffsetY = 1;
      const iconSize = 12;

      // Equipo A
      const teamAGroup = group.append('g').attr('transform', `translate(${itemPadding.x}, ${itemPadding.y_top_center})`);
      if (teamA.logo && teamA.name !== 'TBD') {
        teamAGroup.append('image')
      ...
      // Equipo B
      const teamBGroup = group.append('g').attr('transform', `translate(${itemPadding.x}, ${itemPadding.y_bottom_center})`);
      if (teamB.logo && teamB.name !== 'TBD') {
        teamBGroup.append('image')
